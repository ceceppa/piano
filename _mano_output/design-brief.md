...
- **Markers (non-colour paired):** Root = gold diamond (`--color-tertiary` fill, 2px `#4E3D00` stroke → 10.55:1 ✓) + the key's 3px double-border state. **The root diamond is the largest key marker (18px)** so it reads at a glance against the chord-tone dot (12px) and scale-note ring (14px) — the size + the strongest border carry the root identity, never colour alone. Chord tone = `--color-secondary` dot (≥ 7:1 on white ✓). Scale note = `--color-secondary` at 30% ring + the key's dashed-border state. Active key = light secondary tint (`#DDE1FF` light / `#2E5BFF` dark) with inset shadow.
...
- **Root note + chord quality are the dominant control pair**, placed together at the top in `--color-on-surface` with `--text-body-md` labels; root-note `SegmentedControl` (`--text-notation-lg`) and quality `Select` — both visually stronger (larger text, no card). **Secondary/context controls** (scale-follow, view mode, octave range, note names, genre) follow as a quieter `--text-body-sm` `--color-on-surface-variant` row on `--color-surface-low`. No large bordered `Card` around the control area; spacing + grouping carry the hierarchy.
