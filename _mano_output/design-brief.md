...
Visual language: **Ivory & Ink** — High-Precision Minimalist. Clean lines, generous whitespace, tonal layers instead of heavy shadows; the piano keyboard is always the highest-contrast element on screen. Origin: `ui/ivory_ink/DESIGN.md`; phase-1 screen reference: `ui/explore_chords/code.html`; Explore-redesign reference (ui-gap, phase-4 review Q2): `ui/newone.jpeg` mockup + `ui/newui.md` → Visual direction.
...
|---|---|---|---|
| `--color-background` | `#F7F2E7` | `#202224` | page background |
...
`--color-background` light value corrected to a true warm ivory (ui-gap revision; was a cool grey that didn't match the named "Ivory & Ink" language) — confirmed by the Explore-redesign reference. Dark value unchanged pending a dark-mode reference.
...
- Grouped buttons sharing outer `DEFAULT` radius; selected option `--color-primary` fill + `--color-on-primary` text — canonical since the ui-gap revision (the alternate `--color-surface-high` tint + `--color-secondary` border option is retired, so "selected" and "chord tone" stay visually distinct), never colour alone; `:focus-visible` ring on each option.
...
- Each tile: chord symbol in `--text-notation-lg` `--color-on-surface`, short explanatory label in `--text-body-sm` `--color-on-surface-variant`, `--color-surface-low` fill, 1px `--color-outline-variant` border, radius `DEFAULT`, min 88px height, responsive grid (auto-fill, min 120px columns). Selected state (ui-gap revision, supersedes blue for this role): `--color-primary` fill + `--color-on-primary` text + 2px `--color-primary` outline (non-colour cue), never colour alone. `:focus-visible` 2px `--color-secondary` ring. `--color-secondary` (blue) is now reserved for the chord-tone musical role so it never doubles as the generic "selected" cue.
- **phase-5:** all three groups render together under their headings, with no filter row above them and no See-all control. Hidden entirely in Scale view, which keeps its chord selection.

### Scale-type list — "Explore [root] scales" (phase-5 addition)
- The scale-type picker, in the same tile language as the chord grid so the two read as one pattern: `--color-surface-low` fill, 1px `--color-outline-variant` border, radius `DEFAULT`, min 88px height, responsive grid (auto-fill, min 120px columns), `--color-primary` fill + `--color-on-primary` text + 2px `--color-primary` outline when selected, `:focus-visible` 2px `--color-secondary` ring. Identical states, so a person learns one interaction and reuses it.
- Each tile: the scale name in `--text-notation-lg` `--color-on-surface` ("Major", "Natural minor") above a short explanatory line in `--text-body-sm` `--color-on-surface-variant`.
- Section title "Explore [root] scales" in `--text-headline-lg`, matching the chord section. Shown in Scale and Both views; hidden in Chord view, which keeps its scale selection.
...
### Chromatic root selector (ui-gap revision)
- Twelve buttons in one row, own component (not the shared `SegmentedControl`, since each button carries two stacked labels). Each shows both enharmonic names stacked (e.g. "C♯" over "D♭") in `--text-label-mono` until a notation preference exists. Selected: `--color-primary` fill, `--color-on-primary` text. Unselected: `--color-surface` fill, 1px `--color-outline-variant` border. Same row tier and visual weight as the view selector and inversion selector below — root is not visually subordinate to either.

### View selector — "Show on piano" (ui-gap revision)
- A `SegmentedControl` (Chord / Scale / Both), left of the root selector, same tier. This supersedes the phase-2/3 placement of view mode inside the quieter secondary row: it is now a primary control, not a secondary one.

### Inversion selector (ui-gap revision, supersedes the phase-4 Arrangement-controls pairing)
- Compact buttons (Root / 1st / 2nd / 3rd …), same tier and selected-state treatment as the root and view selectors. A small circular info affordance (16px, `--color-on-surface-variant`, "?" glyph) sits beside the label and reveals "Changes which chord note is lowest." in a `--text-body-sm` tooltip on hover or tap. Voicing and hand arrangement no longer sit inline beside Inversion — they move into the Chord options panel below.

### Chord options panel (ui-gap addition)
- A ghost `Button` labelled "Chord options" in the playback row opens a panel holding the Voicing and Arrangement `SegmentedControl`s (styling unchanged from phase-4). Desktop: anchored popover below the button, `--color-surface` fill, 1px `--color-outline-variant` border, radius `xl`, same elevation as `Card`. Tablet/mobile: bottom sheet, same fill/border, `xl` radius on the top corners only. Closing returns focus to the "Chord options" button.

### Legend (ui-gap addition)
- A compact `--text-body-sm` list beside the piano: one row per marker role currently visible, each a small swatch (matching that marker's piano shape and colour) plus its label in `--color-on-surface-variant`. Shows only the roles the active view uses — Chord: Root, Chord note. Scale: Root, Scale note. Both: all four, including Chord + scale.

### Understand section (ui-gap revision, supersedes the phase-3 Notes panel)
- Renamed from "NOTES" to "Understand [chord/scale name]"; same `Card` treatment and `--text-label-mono` header style. Mode-aware body: Chord view lists chord notes plus one plain-language sentence; Scale view lists scale notes plus one sentence and never repeats chord content; Both view adds a "Shared notes" row. Two `--text-body-sm` links, "Show intervals" / "Show scale degrees", expand an inline detail row without leaving the card.

### Category group labels (phase-5 revision, supersedes the ui-gap Category filter)
- Every supported option in each Explore list is visible at once. Groups are separated by inline `--text-label-mono` uppercase headings in `--color-on-surface-variant` — "COMMON / COLOUR / SEVENTHS & EXTENSIONS" for chords, and a single "COMMON" for scales while only major and natural minor are supported. Nothing is hidden and nothing filters.
- The interactive filter pill described at the ui-gap revision is deferred, together with the Advanced / See-all catalogue it belongs to: it arrives when either list grows long enough to crowd the piano. Until then a filter would hide options a person can already take in at a glance.

## Screen Composition

### phase-5 — Explore (clarity redesign; builds the composition the phase-4 ui-gap set as the target, supersedes phase-4 below)
1. **Header:** app title + the light/dark `ToggleSwitch`, which stays until a Settings screen owns the preference. No settings gear and no navigation this phase — neither destination exists yet.
2. **Primary control row:** view selector (Chord/Scale/Both), chromatic root selector, and inversion selector (Chord/Both views only) — one visual tier, `--color-primary` selected-state throughout, directly above the selection summary.
3. **Selection summary:** the mode-aware title (e.g. "G6 with G major scale") in `--text-headline-xl`, quiet state chips beneath it in Both view, and the Legend on the same row, right-aligned.
4. **Piano keyboard band:** unchanged structure from phase-4 (`--color-surface` band, keys + markers), plus the chord+scale shared-note triangle marker and the teal scale-ring colour (see Component guide).
5. **Playback row:** primary "Hear [chord/scale]" button, one mode-aware secondary action (Arpeggiate or Play descending), a ghost "Chord options" button (Chord/Both views only) opening the Chord options panel, and a state-summary caption ("Close voicing · Both hands") in place of the old static "Ready" label.
6. **Understand card:** supersedes the phase-3 Notes panel (see Component guide).
7. **Explore chord types / Explore scales:** each is its section title, then group headings and the full tile grid — no filter row, no See-all, every supported option visible (see Component guide). Both view stacks both sections below the piano, chord types first.
